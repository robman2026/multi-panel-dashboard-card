...
function renderIcon(iconName, color, size) {
  ...
  // If it looks like an mdi: icon name (with or without prefix), use ha-icon
  // which resolves the full MDI library including pi-hole, etc.
  const hasMdiPrefix = iconName.startsWith('mdi:');
  const knownLocal   = MDI[iconName.replace('mdi:', '')];
  if (hasMdiPrefix || !knownLocal) {
    const mdiName = hasMdiPrefix ? iconName : 'mdi:' + iconName;
    return `<ha-icon icon="${mdiName}" style="color:${color};--mdc-icon-size:${size}px;display:inline-flex;align-items:center;justify-content:center;width:${size}px;height:${size}px;"></ha-icon>`;
  }
...
class MultiPanelDashboardCard extends HTMLElement {
  ...
  _buildHTML() {
    ...
    // ── Right: Salt ──
    // Salt % can come from:
    //   1. A dedicated % entity (salt_pct_entity)
    //   2. salt_max_entity — compute val/max
    //   3. salt_max_value  — a fixed max number in config (e.g. 0.31 for 31cm max)
    //   4. Fallback: treat raw value as % directly if between 0-100
    ...
    if (saltPctEnt !== null) {
      ...
      saltPct = saltVal; // already 0-1
    } else if (saltVal > 1 && saltVal <= 100) {
      saltPct = saltVal / 100; // looks like a percentage
    } else {
      saltPct = 0;
    }
  ...
  // Waits for the custom element to be defined AND uses rAF to ensure upgrade.
  _initStreams() {
  ...
  _tabSalt() {
    ...
    return html`
      <div class="section">
        ...
        <p class="hint">
          The card needs to know the % level. Provide EITHER a % entity, OR a max entity/value to compute it from the distance reading.
        </p>
        <label class="ed-label">Salt distance entity (e.g. sensor.salt_level_distance)</label>
        ${this._entitySearch("salt_ent", cfg.salt_entity,
            v => this._set("salt_entity", v),
            ["sensor"], "— select salt distance sensor —")}
        <label class="ed-label">Salt % entity (optional — if your integration provides one directly)</label>
        ${this._entitySearch("salt_pct", cfg.salt_pct_entity,
            v => this._set("salt_pct_entity", v),
            ["sensor"], "— select % sensor —")}
        <label class="ed-label">Salt max entity (optional — distance at 100%)</label>
        ${this._entitySearch("salt_max", cfg.salt_max_entity,
            v => this._set("salt_max_entity", v),
            ["sensor"], "— select max distance sensor —")}
        ${this._txt("Salt max value (fixed, e.g. 0.31 for 31cm max)", cfg.salt_max_value, v => this._set("salt_max_value", v), "e.g. 0.31")}
